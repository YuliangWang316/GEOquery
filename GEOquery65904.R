library(GEOquery)
gds <- getGEO("GSE65904")
exp<-cbind(gds$GSE65904_series_matrix.txt.gz@assayData$exprs,as.data.frame(gds$GSE65904_series_matrix.txt.gz@featureData@data$Symbol))
exp<-na.omit(exp)
...
clinicaldata<-as.data.frame(cbind(gds$GSE65904_series_matrix.txt.gz@phenoData@data$characteristics_ch1.6,gds$GSE65904_series_matrix.txt.gz@phenoData@data$characteristics_ch1.7))
rownames(clinicaldata)<-rownames(gds$GSE65904_series_matrix.txt.gz@phenoData@data)
